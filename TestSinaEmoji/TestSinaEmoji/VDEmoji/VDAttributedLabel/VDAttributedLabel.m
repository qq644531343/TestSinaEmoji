...
#import "VDAttributedLabelURL.h"

static NSString* const kEllipsesCharacter = @"\u2026";

static dispatch_queue_t VD_attributed_label_parse_queue;
static dispatch_queue_t get_VD_attributed_label_parse_queue() \
{
    if (VD_attributed_label_parse_queue == NULL) {
        VD_attributed_label_parse_queue = dispatch_queue_create("com.VD.parse_queue", 0);
    }
    return VD_attributed_label_parse_queue;
}

@interface VDAttributedLabel ()
{
...
@property (nonatomic,strong)    VDAttributedLabelURL *touchedLink;
@property (nonatomic,assign)    BOOL linkDetected;
@end

@implementation VDAttributedLabel
...
- (void)setFont:(UIFont *)font
{
    if (font && _font != font)
    {
        ...
        [self resetFont];
        for (VDAttributedLabelAttachment *attachment in _attachments)
        {
...
- (NSAttributedString *)attributedStringForDraw
{
    if (_attributedString)
    {
        ...
        for (VDAttributedLabelURL *url in _linkLocations)
        {
...
- (VDAttributedLabelURL *)urlForPoint: (CGPoint)point
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        if (CGRectContainsPoint(rect, point))
        {
            ...
            VDAttributedLabelURL *url = [self linkAtIndex:idx];
            if (url)
            {
                return url;
            }
...
{
    VDAttributedLabelURL *url = [self urlForPoint:point];
...
- (VDAttributedLabelURL *)linkAtIndex:(CFIndex)index
{
    for (VDAttributedLabelURL *url in _linkLocations)
    {
...
- (void)appendAttachment: (VDAttributedLabelAttachment *)attachment
{
    attachment.fontAscent                   = _fontAscent;
    attachment.fontDescent                  = _fontDescent;
             ...
             margin: (UIEdgeInsets)margin
{
    [self appendImage:image
              maxSize:maxSize
               margin:margin
            alignment:VDImageAlignmentBottom];
}
...
{
    VDAttributedLabelAttachment *attachment = [VDAttributedLabelAttachment attachmentWith:image
                                                                                     margin:margin
                                                                             alignment:alignment
                                                                               maxSize:maxSize];
...
- (void)appendView: (UIView *)view
         ...
         alignment: (VDImageAlignment)alignment
{
    VDAttributedLabelAttachment *attachment = [VDAttributedLabelAttachment attachmentWith:view
                                                                                     margin:margin
                                                                                  alignment:alignment
                                                                                    maxSize:CGSizeZero];
    [self appendAttachment:attachment];
...
{
    VDAttributedLabelURL *url = [VDAttributedLabelURL urlWithLinkData:linkData
                                                                  range:range
                                                                  color:color];
...
{
    [VDAttributedLabelURL setCustomDetectMethod:block];
}
...
- (void)drawAttachments
{
    ...
    for (CFIndex i = 0; i < numberOfLines; i++)
    {
        ...
        for (CFIndex k = 0; k < runCount; k++)
        {
            ...
            VDAttributedLabelAttachment* attributedImage = (VDAttributedLabelAttachment *)CTRunDelegateGetRefCon(delegate);
...
- (BOOL)onLabelClick:(CGPoint)point
{
    ...
    if (linkData)
    {
        if (_delegate && [_delegate respondsToSelector:@selector(VDAttributedLabel:clickedOnLink:)])
        {
            [_delegate VDAttributedLabel:self clickedOnLink:linkData];
        }
        else
        {
            NSURL *url = nil;
...
- (void)recomputeLinksIfNeeded
{
    ...
    BOOL sync = length <= VDMinAsyncDetectLinkLength;
               ...
               sync:(BOOL)sync
{
    ...
    LinkBlock block = ^(NSArray *links)
    {
        ...
        if ([links count])
        {
            for (VDAttributedLabelURL *link in links)
            {
    ...
    if (sync)
    {
        NSArray *links = [VDAttributedLabelURL detectLinks:text];
        block(links);
    }
    else
    {
        dispatch_sync(get_VD_attributed_label_parse_queue(), ^{
...
- (void)addAutoDetectedLink: (VDAttributedLabelURL *)link
{
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    VDAttributedLabelURL *touchedLink = [self urlForPoint:point];
